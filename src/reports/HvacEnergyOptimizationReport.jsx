...
export default function HvacEnergyOptimizationReport({ data }) {
  ...
  const pdfDetailedAnnualRows = [
    ...
    { key: 'reheatKWh', label: tr('Énergie annuelle de réchauffage Humifog', 'Annual Humifog reheat energy'), cost: false },
  ...
  const hourlyWeatherSummaryRows = hourlyWeatherDataSummary
    ? [
      ...
      [tr('Enregistrements EPW disponibles (fichier complet)', 'EPW records available (complete file)'), `${formatNumber(hourlyWeatherDataSummary.recordsLoaded || 0, 0)}`],
      [tr('Heures d’exploitation utilisées pour le calcul annuel', 'Operating hours used for annual calculation'), `${formatNumber(hourlyWeatherDataSummary.operatingHoursUsed || 0, 0)} h/an`],
  ...
  const reportInputValidationRows = [
    ...
    ...(isHourlyCalculation ? [
      ...
      [tr('Enregistrements EPW disponibles (fichier complet)', 'EPW records available (complete file)'), `${mode.hourlyWeatherRecordsLoaded || 0}`],
      [tr('Heures d’exploitation utilisées pour le calcul annuel', 'Operating hours used for annual calculation'), `${mode.hourlyWeatherOperatingHoursUsed || 0} h/an`],
